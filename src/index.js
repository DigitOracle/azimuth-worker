...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/poll_send" && request.method === "POST") {   // v105 - a yes/no poll for Naj: one button question per row, answers kept in KV poll_<id>
      ...
      const pid = String(pj.id || "").replace(/[^a-z0-9_-]/gi, "").slice(0, 40); const qs = Array.isArray(pj.questions) ? pj.questions.slice(0, 20) : [];
      if (!pid || !qs.length) return new Response("id and questions required", { status: 400 });
      const rec = { id: pid, sent: new Date().toISOString(), intro: pj.intro || "", outro: pj.outro || "", questions: qs, answers: {} };
      await env.MEETINGS.put("poll_" + pid, JSON.stringify(rec));
      try {
        if (pj.intro) await waSend(env, env.WA_ALLOWED, String(pj.intro).slice(0, 3500));
        for (const q of qs) {
          await waSendButtons(env, env.WA_ALLOWED, String(q.text).slice(0, 1000), [
            { id: "bq:" + pid + ":" + q.n + ":y", title: "Yes" }, { id: "bq:" + pid + ":" + q.n + ":n", title: "No" }, { id: "bq:" + pid + ":" + q.n + ":u", title: "Not sure" }]);
        }
    ...
    if (url.pathname === "/poll_status") {
      ...
      const rec = await env.MEETINGS.get("poll_" + pid);
    ...
    if (request.method === "POST") {
      ...
      if (url.pathname === "/wa") {
        ...
        if (msg.type === "interactive" && msg.interactive && (msg.interactive.button_reply || msg.interactive.list_reply)) {
          ...
          if (bid.indexOf("bq:") === 0) {                                              // v105 - poll answer
            const _pp = bid.split(":"); const _pid = _pp[1], _qn = _pp[2], _ans = _pp[3];
            let _pr = null; try { _pr = JSON.parse((await env.MEETINGS.get("poll_" + _pid)) || "null"); } catch (e) {}
            if (_pr) {
              _pr.answers[_qn] = { ans: _ans === "y" ? "yes" : (_ans === "n" ? "no" : "not sure"), at: new Date().toISOString() };
              await env.MEETINGS.put("poll_" + _pid, JSON.stringify(_pr));
...
const FEED_FAMILIES = ["prices", "rents_yields", "volume", "handover_supply", "developer", "district", "offplan_ready", "news", "transit", "buyer_maths", "city_life", "inventory", "education"];   // v126 - city_life, inventory
const FAM_RX = [["inventory", /released|taken up|availability sheet|still available|units? (?:left|remaining)|claims? .{0,20}available/i],   // v126
  ["city_life", /bus stop|coverage|airport|departures|per hour|km\/h|residents|profession|heritage|within reach|no stop/i],   // v126
  ["transit", /metro|rail|tram|station|commute|etihad|route 2020/i], ["handover_supply", /handover|complet|deliver|pipeline|supply|under construction|launch/i],
  ...
  ["developer", /emaar|damac|sobha|nakheel|meraas|ellington|binghatti|select group|omniyat|imtiaz|beyond|fakhruddin|arada|iman|zaya|palma|prestige one|developer/i],
...
async function dailyFeedTick(env, force, dry) {
  ...
  const data = JSON.stringify({
    ...
    cityLife: d.cityLife ? (() => {                                              // v126 - Dubai as a place to live
      const c = d.cityLife, o = {};
      if (c.metro) o.metro = { stations: c.metro.stations, byLine: c.metro.byLine, districtsMeasured: c.metro.districtsMeasured, withinWalk2km: c.metro.withinWalk2km, beyond5km: c.metro.beyond5km, closest: (c.metro.closest || []).slice(0, 5), furthest: (c.metro.furthest || []).slice(-4) };
      if (c.busCoverage) o.busCoverage = { reportDate: c.busCoverage.reportDate, communities: c.busCoverage.communities, populationCovered: c.busCoverage.populationCovered, stops: c.busCoverage.stops, cityCoveragePct: c.busCoverage.cityCoveragePct, communitiesOver1000WithNoStop: c.busCoverage.communitiesOver1000WithNoStop, worstServed: (c.busCoverage.worstServed || []).slice(0, 4), bestServed: (c.busCoverage.bestServed || []).slice(0, 3), theValley: c.busCoverage.theValley };
      if (c.busStops) o.busStops = { total: c.busStops.total, radiusKm: c.busStops.radiusKm, mostServed: (c.busStops.mostServed || []).slice(0, 4), leastServed: (c.busStops.leastServed || []).slice(-4) };
      if (c.airport) o.airport = c.airport;
      if (c.buses) o.buses = { medianKph: c.buses.medianKph, observations: c.buses.observations, linesMeasured: c.buses.linesMeasured, slowest: (c.buses.slowest || []).slice(0, 2), fastest: (c.buses.fastest || []).slice(-2) };
      if (c.professions) o.professions = c.professions;
      return o;
    })() : null,
    developerInventory: d.developerInventory ? { asOf: d.developerInventory.asOf, basis: d.developerInventory.basis, caution: d.developerInventory.caution, current: d.developerInventory.current, moves: (d.developerInventory.moves || []).slice(0, 8) } : null,
    trends,
...
const MAP_CHROME_HTML = ''
  ...
  + '  <div class=hrow><label>extras</label><div class=seg><button data-x=beach type=button>near a public beach</button><button data-x=hbeach type=button>near a hotel beach</button></div></div>'
...
const MAP_CHROME_JS = ''
  ...
  + 'function nearBeach(it,acc){if(!AM||!AM.items)return false;acc=acc||"public";for(var k=0;k<AM.items.length;k++){var a=AM.items[k];if(a.k!=="beach")continue;if(acc==="public"&&a.acc&&a.acc!=="public")continue;if(acc==="hotel"&&a.acc!=="hotel")continue;if(m2([it.lon,it.lat],[a.lon,a.lat])<=1000)return true}return false}'
  ...
  + 'function setWide(){var w=new URLSearchParams(location.search).get("wide");var on=w==="1"||(w!=="0"&&innerWidth>=900);document.body.classList.toggle("wide",on)}setWide();addEventListener("resize",setWide);'
  ...
  + '  if(!f)return;var T=ntok(f);var same=function(n){var U=ntok(n);if(!U.length||!T.length)return false;var Ueq=U.length===T.length&&U.every(function(w){return T.indexOf(w)>=0});if(Ueq)return true;var UinT=U.every(function(w){return T.indexOf(w)>=0}),TinU=T.every(function(w){return U.indexOf(w)>=0});return (UinT&&U.length>=Math.max(2,T.length-1))||(TinU&&T.length>=2)};'
  ...
  + 'function flyRefresh(){document.querySelectorAll("u[data-fly]").forEach(function(u){var on=flyOn(u.getAttribute("data-fly"));u.style.opacity=on?"1":".3";u.style.cursor=on?"pointer":"default";u.title=on?"UnReal":"UnReal \u00b7 coming soon"})}'
  ...
  + 'function wireFly(el){el.querySelectorAll("u[data-fly]").forEach(function(u){u.onclick=function(e){e.stopPropagation();var slug=u.getAttribute("data-fly");var v=flyFor(slug);if(v){openFly(v);return}if(WALK&&WALK.live&&WALK.url){window.open(WALK.url+"/?StreamerId="+encodeURIComponent(WALK.streamer||"NajmaDubai")+"&AutoConnect=true&AutoPlayVideo=true&StartVideoMuted=true&HoveringMouse=true&MatchViewportRes=true","_blank","noopener")}}})}'
  ...
  + 'function umxTable(r){if(!r||!r.rows)return \'\';var ac=r.asset_classes||{};var res=(r.total_units||0)-(ac.office||0)-(ac.retail||0);var h=\'<div class=umx><div class=ps>unit mix \u00b7 \'+(r.total_units||0)+\' units \u00b7 \'+(r.basis||\'DLD units register\')+\'</div><div class=k><b>\'+res+\'<small>residential</small></b>\'+(ac.retail?\'<b>\'+ac.retail+\'<small>retail</small></b>\':\'\')+(ac.office?\'<b>\'+ac.office+\'<small>office</small></b>\':\'\')+\'</div><table><tr><th>type</th><th>units</th><th>size</th><th>levels</th><th>sold @</th><th>rent</th></tr>\';r.rows.forEach(function(x){h+=\'<tr><td>\'+esc(x.type||\'\')+\'</td><td>\'+(x.units||\'\')+\'</td><td>\'+(x.median_sqm?Math.round(x.median_sqm)+\' m\u00b2\':\'\u00b7\')+\'</td><td>\'+esc((x.levels||\'\u00b7\').split(\' (\')[0])+\'</td><td>\'+(x.median_aed?fmtM(x.median_aed):\'\u00b7\')+\'</td><td>\'+(x.median_rent?fmtM(x.median_rent)+\'/yr\':\'\u00b7\')+\'</td></tr>\'});h+=\'</table><div class=ps style="text-transform:none;letter-spacing:0;margin-top:6px">\'+(r.floors?r.floors+\' floors \u00b7 \':\'\')+(r.car_parks?r.car_parks+\' car parks \u00b7 \':\'\')+(r.register_aed_sqm?\'AED \'+Math.round(r.register_aed_sqm).toLocaleString(\'en\')+\'/m\u00b2 in the register\':\'\')+\'</div></div>\';return h}'
  ...
  + '  var ref=refPoint();Object.keys(AMEN).forEach(function(k){var nA=0,nD=0;(AM&&AM.items?AM.items:[]).forEach(function(i){if(i.k!==k)return;if(inArea(i,dd))nA++;if(inDist(i,ref,RKM))nD++});var c=document.getElementById("amc-"+k),sm=document.getElementById("ams-"+k);var isPB=(k==="beach"||k==="park");var nAp=0,nDp=0;if(isPB){(AM&&AM.items?AM.items:[]).forEach(function(i){if(i.k!==k||(i.acc&&i.acc!=="public"))return;if(inArea(i,dd))nAp++;if(inDist(i,ref,RKM))nDp++})}if(c)c.textContent=String(isPB?(MODE==="area"?nAp:nDp):(MODE==="area"?nA:nD));if(sm){var txt=(dd||SEL)?(MODE==="area"?nD+" within "+RKM+" km":(dd?nA+" in community":"")):"";if(isPB){var other=(MODE==="area"?nA-nAp:nD-nDp);txt=(other>0?other+" private":"")+(dd&&other>0?" \u00b7 ":"")+(dd?(MODE==="area"?nDp+" public within "+RKM+" km":nAp+" public in community"):"")}if(k==="beach"&&SEL&&!(MODE==="area"?nA:nD)){var wl2=waterLine(SEL.c);if(wl2)txt=wl2}sm.textContent=txt}});syncChips();'
  ...
  + '  if(!keys.length){["school","hospital","clinic","metro","mall","park","beach"].forEach(function(k){var best=null,bestPub=null;(AM&&AM.items?AM.items:[]).forEach(function(i){if(i.k!==k)return;var dd=m2(c,[i.lon,i.lat]);if(!best||dd<best[0])best=[dd,i];if((!i.acc||i.acc==="public"||i.acc==="community")&&(!bestPub||dd<bestPub[0]))bestPub=[dd,i]});if(k==="park"||k==="beach"){if(bestPub)near.push(bestPub);if(best&&(!bestPub||best[1]!==bestPub[1])&&best[0]<(bestPub?bestPub[0]:1e9))near.push(best)}else if(best)near.push(best)});setNear(near);'
  ...
  + '  var bld=(hm?homeBlock(hm):\'\')+(ur?umxTable(ur):\'\')+((!hm&&!ur)?\'<div class=ps style="text-transform:none;letter-spacing:0;margin-top:6px">no priced record under this name in the register yet \u00b7 <a href="/find?key=\'+encodeURIComponent(KEY)+\'&q=\'+encodeURIComponent(p.name)+\'">look it up in Find \u2192</a></div>\':\'\')+(slug&&!window.__twinDistrict?\'<div class=ps style="margin-top:8px"><a href="/skyline/\'+encodeURIComponent(slug)+\'?key=\'+encodeURIComponent(KEY)+(twinI!=null?\'&b=\'+twinI:\'\')+\'">on the twin \u2192</a></div>\':\'\');'
...
function renderCity(key) {
  ...
  const css = 'html,body{margin:0;height:100%;background:#0C1413;color:#E8E4D8;font-family:"IBM Plex Sans",system-ui,sans-serif;overflow:hidden}'
    ...
    + '#sb{position:fixed;left:14px;top:66px;z-index:8;width:min(420px,calc(100vw - 28px))}#sb input{width:100%;box-sizing:border-box;background:rgba(12,20,19,.92);border:1px solid #24352F;border-radius:12px;color:#E8E4D8;font:15px "IBM Plex Sans",system-ui,sans-serif;padding:10px 14px;outline:none}#sb input:focus{border-color:#C5A56A}'
    + '#sr{margin-top:6px;background:rgba(12,20,19,.96);border:1px solid #24352F;border-radius:12px;overflow:hidden;display:none}#sr div{padding:9px 14px;cursor:pointer;display:flex;justify-content:space-between;gap:10px;border-top:1px solid #1B2624}#sr div:first-child{border-top:0}#sr div:hover,#sr div.on{background:#16302B}#sr b{font-weight:500;color:#E8E4D8}#sr s{text-decoration:none;font-family:"IBM Plex Mono",monospace;font-size:10px;letter-spacing:.08em;text-transform:uppercase;color:#8FA39E;white-space:nowrap}'
    + '#sel{margin-top:6px;width:100%;background:rgba(12,20,19,.92);border:1px solid #24352F;border-radius:12px;color:#C5A56A;font:12px "IBM Plex Mono",monospace;letter-spacing:.06em;padding:8px 12px}'
  ...
  const js = [
    ...
    'scene.add(new THREE.HemisphereLight(0xe8efec,0x243230,1.7));const sun=new THREE.DirectionalLight(0xffe9c4,1.8);sun.position.set(-20000,30000,-12000);scene.add(sun);',
    ...
    'mesh=new THREE.InstancedMesh(new THREE.BoxGeometry(1,1,1),new THREE.MeshStandardMaterial({roughness:.6,metalness:.05,emissive:0x0b1614,emissiveIntensity:.35}),n);',
    'for(let i=0;i<n;i++){const x=xz[2*i]*U,z=xz[2*i+1]*U,w=wd[2*i],d=wd[2*i+1],h=hh[i];const hh2=Math.max(h,8);tmp.position.set(x,hh2/2,z);tmp.scale.set(Math.max(w,6),hh2,Math.max(d,6));tmp.updateMatrix();mesh.setMatrixAt(i,tmp.matrix);',
    '  const f=fl[i];const tower=f&8,villa=f&2,town=f&4;col.setHSL(hues(di[i]),tower?.45:.28,tower?.78:(villa||town?.46:.56));mesh.setColorAt(i,col)}',
    ...
    'const box=new THREE.Box3().setFromObject(mesh);const ctr=box.getCenter(new THREE.Vector3());const size=box.getSize(new THREE.Vector3());ctl.target.copy(ctr).setY(0);cam.position.set(ctr.x+size.x*.22,Math.max(size.x,size.z)*.34,ctr.z+size.z*.48);cam.lookAt(ctl.target);',
    ...
    'const q=document.getElementById("q"),sr=document.getElementById("sr"),sel=document.getElementById("sel");const cors={};D.districts.forEach((d,ix)=>{(cors[d.corridor||"Other"]=cors[d.corridor||"Other"]||[]).push(ix)});'+
    'Object.keys(cors).sort().forEach(c=>{const og=document.createElement("optgroup");og.label=c;cors[c].sort((a,b)=>D.districts[a].name.localeCompare(D.districts[b].name)).forEach(ix=>{const o=document.createElement("option");o.value=ix;o.textContent=D.districts[ix].name+" \u00b7 "+D.districts[ix].buildings.toLocaleString("en");og.appendChild(o)});sel.appendChild(og)});'+
    'sel.onchange=()=>{if(sel.value!==""){flyTo(+sel.value,true)}};'+
    'function norm(t){return (t||"").toLowerCase().replace(/[^a-z0-9 ]/g," ").replace(/\\s+/g," ").trim()}'+
    'function show(list){sr.innerHTML=list.map((ix,k)=>"<div data-ix=\\""+ix+"\\""+(k===0?" class=on":"")+"><b>"+D.districts[ix].name+"</b><s>"+(D.districts[ix].corridor||"")+" \u00b7 "+D.districts[ix].buildings.toLocaleString("en")+" buildings</s></div>").join("");sr.style.display=list.length?"block":"none";sr.querySelectorAll("div").forEach(el=>{el.onclick=()=>{sr.style.display="none";q.value=D.districts[+el.dataset.ix].name;flyTo(+el.dataset.ix,true)}})}'+
    'q.oninput=()=>{const v=norm(q.value);if(!v){sr.style.display="none";return}const toks=v.split(" ");const list=D.districts.map((d,ix)=>({ix,s:toks.reduce((a,t)=>a+((norm(d.name)+" "+norm(d.corridor)+" "+d.slug).indexOf(t)>=0?1:0),0)})).filter(x=>x.s===toks.length).map(x=>x.ix).slice(0,8);show(list);const hitIx=list[0];labels.forEach((l,i)=>l.e.classList.toggle("on",i===hitIx));if(hitIx!=null){ctl.autoRotate=false}};'+
    'q.onkeydown=(e)=>{if(e.key==="Enter"){const f=sr.querySelector("div");if(f)f.click()}if(e.key==="Escape"){sr.style.display="none";q.blur()}};document.addEventListener("pointerdown",(e)=>{if(!document.getElementById("sb").contains(e.target))sr.style.display="none"});'
  ...
  return '<!doctype html><html lang=en><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1,viewport-fit=cover"><title>Najma \u2014 the twin \u00b7 all Dubai</title>' + NAJ_FONTS
    ...
    + '<div id=hd><h1>Najma <b>\u0646\u062c\u0645\u0629</b> \u2014 the twin \u00b7 all Dubai</h1><p>loading</p></div>'
...
const DEVCOL={omniyat:0xE8E4D8,hh:0xD96C5F,meraas:0xD9A441,select:0x3E8A7E,ellington:0x8FC7B9,arada:0x56B584,zaya:0x9B8CE6,palma:0x6FB7A8,fakhruddin:0xF28C6A,beyond:0x4C9BE8,imtiaz:0xC41E3A,iman:0xB08BD9,emaar:0xC5A56A,sobha:0xE07A5F,prestigeone:0x7FA7C9};
const DEVNAME={omniyat:"OMNIYAT",hh:"H&H",meraas:"Meraas",select:"Select Group",ellington:"Ellington",arada:"Arada",zaya:"ZAYA",palma:"Palma",fakhruddin:"Fakhruddin",beyond:"BEYOND",imtiaz:"Imtiaz",iman:"Iman",emaar:"Emaar",sobha:"Sobha",prestigeone:"Prestige One"};
...
function applyProj(name){
  ...
  if(!mine.size&&name&&SUBB&&SUBB.lc&&window.BYFP){const fps=SUBB.lc[String(name).toLowerCase().trim()]||[];for(const fp of fps){(window.BYFP[fp]||[]).forEach(i=>mine.add(i))}}   // v121
